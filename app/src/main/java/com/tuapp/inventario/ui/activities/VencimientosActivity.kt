...
class VencimientosActivity : AppCompatActivity() {
    ...
    private fun actualizarTextoFecha() {
        try {
            val sdf = DateHelper.getDateFormat("yyyy-MM-dd")
            val sdfDisplay = DateHelper.getDateFormat("dd/MM/yy")
        ...
        lifecycleScope.launch {
            try {
                ...
                if (!stockFinalAyerExiste) {
                    runOnUiThread {
                        val fechaAyerFormateada = DateHelper.formatearFecha(fechaAyer, "dd/MM/yy")
    ...
    private suspend fun calcularStockActualDesdeFirebase(): Map<String, Int> {
        return try {
            ...
            val stockAyerDeduplicado = stockAyer
            ...
            val stockMap = mutableMapOf<String, Int>()
            stockAyerDeduplicado.forEach { (codigo, cantidad) ->
            ...
            val ingresoHoyDeduplicado = ingresoHoy
                .groupBy { obtenerCodigoProducto(it.nombre) }
                .mapValues { (_, prods) ->
                    val prodCompleto = prods.find { it.nombre.contains(" - ") } ?: prods.first()
                    prodCompleto.cantidad
            ...
            ingresoHoyDeduplicado.forEach { (codigo, cantidad) ->
        ...
        if (tieneDatosStock && productosConStockSinLotes.isEmpty() && productosConDiferencias.isEmpty() && hayProductosConLotes) {
            ...
            if (todosTienenLotes) {
                ...
                val txtTodoEnOrden = TextView(this).apply {
                    ...
                    setTextColor(ContextCompat.getColor(this@VencimientosActivity, R.color.web_text_light))
                ...
                if (hayProductosCategoria) {
                    ...
                    val txtSubCategoria = TextView(this).apply {
                        ...
                        val textColor = if (isDarkMode()) {
                            ContextCompat.getColor(this@VencimientosActivity, R.color.web_text_light)
                        } else {
                            ContextCompat.getColor(this@VencimientosActivity, R.color.web_text_primary)
                        }
