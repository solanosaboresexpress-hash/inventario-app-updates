...
import com.tuapp.inventario.ui.activities.AdministracionLocalActivity
import com.tuapp.inventario.data.models.DocumentoLocal
import com.tuapp.inventario.data.models.PersonalLocal
...
class MainActivity : AppCompatActivity() {
    ...
    private lateinit var txtDiaSemana: TextView
    ...
    private lateinit var btnIngreso: LinearLayout
    private lateinit var btnStock: LinearLayout
    private lateinit var btnVencimientos: LinearLayout
    private lateinit var btnPedidoFabrica: LinearLayout
    private lateinit var txtSubIngreso: TextView
    private lateinit var txtSubStock: TextView
    private lateinit var txtSubVenc: TextView
    private lateinit var txtSubPedido: TextView
    private lateinit var txtStatusIngreso: TextView
    private lateinit var txtStatusStock: TextView
    private lateinit var txtStatusVenc: TextView
    private lateinit var txtStatusPedido: TextView
    private lateinit var iconStatusIngreso: ImageView
    private lateinit var iconStatusStock: ImageView
    private lateinit var iconStatusVenc: ImageView
    private lateinit var iconStatusPedido: ImageView
    ...
    private var ultimoEstadoPedidoFabrica: Boolean? = null
    ...
    private val actualizarBotonesReceiver = object : BroadcastReceiver() {
        override fun onReceive(context: Context?, intent: Intent?) {
            Log.d("MainActivity", "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
            Log.d("MainActivity", "📡 BROADCAST RECIBIDO EN MAINACTIVITY")
            ...
            if (intent?.action == "com.tuapp.inventario.ACTUALIZAR_BOTONES") {
                ...
                Log.d("MainActivity", "✅ Acción coincide: ACTUALIZAR_BOTONES")
                Log.d("MainActivity", "📡 Broadcast recibido: Actualizar botones - tipo: $tipo")
                Log.d("MainActivity", "   LocalId: $localIdBroadcast")
                ...
                lifecycleScope.launch {
                    ...
                    // Verificar si tipo es "Stock Final" (simplificado - sin validar localId)
                    // Esto asegura que siempre se calculen las tendencias al guardar Stock Final
                    if (tipo == "Stock Final") {
                        Log.d("TENDENCIAS", "✅ Tipo es Stock Final → Calculando tendencias...")
                        Log.d("TENDENCIAS", "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
                        calcularYGuardarTendencias()
                    } else {
                        Log.d("TENDENCIAS", "⏭️ Tipo NO es Stock Final ($tipo) → No se calculan tendencias")
                        Log.d("TENDENCIAS", "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
                    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        txtSubIngreso = findViewById(R.id.txtSubIngreso)
        txtSubStock = findViewById(R.id.txtSubStock)
        txtSubVenc = findViewById(R.id.txtSubVenc)
        txtSubPedido = findViewById(R.id.txtSubPedido)
        txtStatusIngreso = findViewById(R.id.txtStatusIngreso)
        txtStatusStock = findViewById(R.id.txtStatusStock)
        txtStatusVenc = findViewById(R.id.txtStatusVenc)
        txtStatusPedido = findViewById(R.id.txtStatusPedido)
        iconStatusIngreso = findViewById(R.id.iconStatusIngreso)
        iconStatusStock = findViewById(R.id.iconStatusStock)
        iconStatusVenc = findViewById(R.id.iconStatusVenc)
        iconStatusPedido = findViewById(R.id.iconStatusPedido)

        val btnPromedioVentas: LinearLayout = findViewById(R.id.btnPromedioVentas)
        val btnGuiaAyuda: TextView = findViewById(R.id.btnGuiaAyuda)
        btnGuiaAyuda.setOnClickListener {
            com.tuapp.inventario.ui.dialogs.InteractiveGuideHelper.show(this, fullTour = true)
        }
        
        // Configurar fecha actual
        configurarFechaActual()
        
        // Auto-iniciar guía interactiva en el primer inicio de sesión
        com.tuapp.inventario.ui.dialogs.InteractiveGuideHelper.maybeAutoStart(this)
        ...
        // Botón Administración del Local
        val btnAdministracionLocal: LinearLayout = findViewById(R.id.btnAdministracionLocal)
        btnAdministracionLocal.setOnClickListener {
            val intent = Intent(this, AdministracionLocalActivity::class.java)
            intent.putExtra("localId", usuarioActual?.localId ?: "")
            intent.putExtra("localNombre", txtLocalActual.text.toString().replace("Administración: ", "").replace("Administración: ", ""))
            startActivity(intent)
        }
        
        // Botón Pedido a Fábrica
        btnPedidoFabrica = findViewById(R.id.btnPedidoFabrica)
    ...
    private fun configurarFechaActual() {
        ...
        val sdfDia = DateHelper.getDateFormat("EEEE", Locale("es", "ES"))
        sdfDia.timeZone = timeZone
        val sdfFecha = DateHelper.getDateFormat("dd/MM/yy")
        sdfFecha.timeZone = timeZone
        fechaSeleccionada = DateHelper.getDateFormat("yyyy-MM-dd").also { it.timeZone = timeZone }.format(calendar.time)
        txtFechaActual.text = sdfFecha.format(calendar.time)
        txtDiaSemana.text = sdfDia.format(calendar.time).replaceFirstChar { it.uppercase() }
    }
    ...
    private fun crearDialogoCalendarioConColores(
        ...
        val btnMesAnterior = Button(this).apply {
            ...
            setTextColor(ContextCompat.getColor(this@MainActivity, R.color.web_text_light))
        ...
        val txtMesAño = TextView(this).apply {
            ...
            setTextColor(ContextCompat.getColor(this@MainActivity, R.color.web_text_primary))
        }
    ...
    private fun actualizarFechaSeleccionada(calendar: Calendar) {
        ...
        val sdfFecha = DateHelper.getDateFormat("dd/MM/yy")
        sdfFecha.timeZone = timeZone
        val sdfFechaDb = DateHelper.getDateFormat("yyyy-MM-dd")
        sdfFechaDb.timeZone = timeZone
        fechaSeleccionada = sdfFechaDb.format(calendar.time)
        txtFechaActual.text = sdfFecha.format(calendar.time)
        txtDiaSemana.text = sdfDia.format(calendar.time).replaceFirstChar { it.uppercase() }
    ...
    private fun toggleDarkMode() {
        ...
        // recreate() se llama automáticamente al cambiar night mode
    }
    ...
    private fun verificarRegistrosExistentes() {
        lifecycleScope.launch {
            try {
                ...
                // Consultar si hay pedido a fábrica para esta fecha
                val pedidoFabricaDoc = firestore.collection("locales").document(localId).collection("pedidos_fabrica").document(fechaSeleccionada)
                val pedidoSnapshot = kotlinx.coroutines.withContext(kotlinx.coroutines.Dispatchers.IO) {
                    pedidoFabricaDoc.get().await()
                }
                val tienePedidoFabrica = pedidoSnapshot.exists()
                
                val tieneIngreso = registrosFirebase.contains("Ingreso de Mercadería")
                val tieneStock = registrosFirebase.contains("Stock Final")
                
                // Log para debugging
                Log.d("MainActivity", "🔍 Verificando registros en Firebase - Ingreso: $tieneIngreso, Stock: $tieneStock, Pedido: $tienePedidoFabrica (Fecha: $fechaSeleccionada)")
                ...
                if (tienePedidoFabrica) {
                    // Usar un tipo especial para identificarlo en actualizarIndicadoresBotones
                    registrosFecha.add(RegistroInventario(fechaSeleccionada, "Pedido a Fábrica", emptyList()))
                }
        ...
        runOnUiThread {
            ...
            try {
                ...
                if (ultimoEstadoIngreso != tieneIngreso) {
                    ...
                    if (tieneIngreso) {
                        txtSubIngreso.text = "✅ Completado"
                        txtSubIngreso.setTextColor(ContextCompat.getColor(this@MainActivity, R.color.status_success))
                        txtStatusIngreso.text = "Completado"
                        txtStatusIngreso.setTextColor(ContextCompat.getColor(this@MainActivity, R.color.status_success))
                        iconStatusIngreso.setImageResource(R.drawable.ic_check_circle)
                        iconStatusIngreso.setColorFilter(ContextCompat.getColor(this@MainActivity, R.color.status_success))
                    } else {
                        txtSubIngreso.text = "Pendiente"
                        txtSubIngreso.setTextColor(ContextCompat.getColor(this@MainActivity, R.color.web_text_secondary))
                        txtStatusIngreso.text = "Pendiente"
                        txtStatusIngreso.setTextColor(ContextCompat.getColor(this@MainActivity, R.color.web_text_primary))
                        iconStatusIngreso.setImageResource(R.drawable.ic_pending)
                        iconStatusIngreso.setColorFilter(ContextCompat.getColor(this@MainActivity, R.color.web_gold_text_light))
                    }
                }
                ...
                if (ultimoEstadoStock != tieneStock) {
                    ...
                    if (tieneStock) {
                        txtSubStock.text = "✅ Completado"
                        txtSubStock.setTextColor(ContextCompat.getColor(this@MainActivity, R.color.status_success))
                        txtStatusStock.text = "Completado"
                        txtStatusStock.setTextColor(ContextCompat.getColor(this@MainActivity, R.color.status_success))
                        iconStatusStock.setImageResource(R.drawable.ic_check_circle)
                        iconStatusStock.setColorFilter(ContextCompat.getColor(this@MainActivity, R.color.status_success))
                    } else {
                        txtSubStock.text = "Pendiente"
                        txtSubStock.setTextColor(ContextCompat.getColor(this@MainActivity, R.color.web_text_secondary))
                        txtStatusStock.text = "Pendiente"
                        txtStatusStock.setTextColor(ContextCompat.getColor(this@MainActivity, R.color.web_text_primary))
                        iconStatusStock.setImageResource(R.drawable.ic_pending)
                        iconStatusStock.setColorFilter(ContextCompat.getColor(this@MainActivity, R.color.web_gold_text_light))
                    }
                }
                
                // Verificar si hay registros de Pedido a Fábrica
                val tienePedidoFabrica = registros.any { it.tipo == "Pedido a Fábrica" }
                
                // Actualizar botón de Pedido a Fábrica si el estado ha cambiado
                if (ultimoEstadoPedidoFabrica != tienePedidoFabrica) {
                    ultimoEstadoPedidoFabrica = tienePedidoFabrica
                    // Log solo cuando hay cambio
                    Log.d("MainActivity", "🔄 Botón Pedido a Fábrica actualizado: ${if (tienePedidoFabrica) "✅" else "⏳"}")
                    if (tienePedidoFabrica) {
                        txtSubPedido.text = "✅ Completado"
                        txtSubPedido.setTextColor(ContextCompat.getColor(this@MainActivity, R.color.status_success))
                        txtStatusPedido.text = "Completado"
                        txtStatusPedido.setTextColor(ContextCompat.getColor(this@MainActivity, R.color.status_success))
                        iconStatusPedido.setImageResource(R.drawable.ic_check_circle)
                        iconStatusPedido.setColorFilter(ContextCompat.getColor(this@MainActivity, R.color.status_success))
                    } else {
                        txtSubPedido.text = "Pendiente"
                        txtSubPedido.setTextColor(ContextCompat.getColor(this@MainActivity, R.color.web_text_secondary))
                        txtStatusPedido.text = "Pendiente"
                        txtStatusPedido.setTextColor(ContextCompat.getColor(this@MainActivity, R.color.web_text_primary))
                        iconStatusPedido.setImageResource(R.drawable.ic_pending)
                        iconStatusPedido.setColorFilter(ContextCompat.getColor(this@MainActivity, R.color.web_gold_text_light))
                    }
                }
        ...
        lifecycleScope.launch {
            try {
                ...
                runOnUiThread {
                    ...
                    if (coincide) {
                        txtSubVenc.text = "✅ Completado"
                        txtSubVenc.setTextColor(ContextCompat.getColor(this@MainActivity, R.color.status_success))
                        txtStatusVenc.text = "Completado"
                        txtStatusVenc.setTextColor(ContextCompat.getColor(this@MainActivity, R.color.status_success))
                        iconStatusVenc.setImageResource(R.drawable.ic_check_circle)
                        iconStatusVenc.setColorFilter(ContextCompat.getColor(this@MainActivity, R.color.status_success))
                    } else {
                        txtSubVenc.text = "Pendiente"
                        txtSubVenc.setTextColor(ContextCompat.getColor(this@MainActivity, R.color.web_text_secondary))
                        txtStatusVenc.text = "Pendiente"
                        txtStatusVenc.setTextColor(ContextCompat.getColor(this@MainActivity, R.color.web_text_primary))
                        iconStatusVenc.setImageResource(R.drawable.ic_pending)
                        iconStatusVenc.setColorFilter(ContextCompat.getColor(this@MainActivity, R.color.web_gold_text_light))
                    }
                }
            ...
            try {
                ...
                runOnUiThread {
                    if (snapshotTotal.isEmpty) {
                        ...
                        lifecycleScope.launch {
                            // 1. Verificar datos en 0 (ultimos 7 dias, excluyendo hoy)
                            ...
                            // 2. Verificar días faltantes en los últimos 7 días (Ingreso de Mercadería / Stock Final)
    ...
    private suspend fun verificarDatosEnCeroHoy(localId: String, @Suppress("UNUSED_PARAMETER") fechaHoy: String, problemas: MutableList<String>) {
        ...
        try {
            ...
            if (isAlertasCacheValid(cacheRegistrosAlertasTimestamp, localId) && cacheRegistrosAlertas != null) {
                ...
                Log.d("MainActivity", "📊 Registros obtenidos (todos los históricos): ${snapshot.documents.size}")
            ...
            // Filtrar solo los ultimos 7 dias (excluyendo hoy)
            val calendarHoy = DateHelper.getCalendar()
            val fechaLimite = DateHelper.getDateFormat("yyyy-MM-dd").format(Date(calendarHoy.timeInMillis - 7L * 24 * 60 * 60 * 1000))
            
            registros.filter { it.fecha != fechaHoy && it.fecha >= fechaLimite }.forEach { registro ->
                ...
                when (registro.tipo) {
                    "Ingreso de Mercadería" -> {
                        ...
                        if (todosEnCero && !registro.noRecibioMercaderia) {
                            ...
                            val problema = "📦 $diaSemana $fechaFormateada:\n Ingreso de Mercadería\n   ⚠️ Todos los productos en 0"
                    ...
                    "Stock Final" -> {
                        ...
                        if (todosEnCero) {
                            ...
                            val diaSemana = obtenerDiaSemana(calendarFecha)
                            val fechaFormateada = formatearFechaParaMostrar(calendarFecha)
                            val problema = "📊 $diaSemana $fechaFormateada - Stock Final\n   ⚠️ Todos los productos en 0"
                            problemas.add(problema)
                            Log.d("MainActivity", "⚠️ Problema encontrado: ${registro.tipo} - ${registro.fecha}")
                        }
                    }
    ...
    private suspend fun verificarDiasFaltantes(localId: String, fechaHoy: String, problemas: MutableList<String>) {
        Log.d("MainActivity", "🔍 Verificando días faltantes en TODOS los registros históricos para local: $localId")
        ...
        try {
            ...
            // Agrupar registros por fecha (excluyendo hoy, solo ultimos 7 dias)
            val formatoFecha = DateHelper.getDateFormat("yyyy-MM-dd")
            val calendarHoy = DateHelper.getCalendar()
            val fechaLimite = formatoFecha.format(Date(calendarHoy.timeInMillis - 7L * 24 * 60 * 60 * 1000))
            val registrosPorFecha = registros
                .filter { it.fecha != fechaHoy && it.fecha >= fechaLimite } // Excluir hoy, solo ultimos 7 dias
                .groupBy { it.fecha }
                .mapValues { (_, registrosFecha) -> registrosFecha.map { it.tipo }.toSet() }
            
            Log.d("MainActivity", "📊 Fechas con datos (excluyendo hoy): ${registrosPorFecha.keys.sorted()}")
            ...
            registrosPorFecha.forEach { (fecha, tiposExistentes) ->
                ...
                if (!tiposExistentes.contains("Ingreso de Mercadería")) {
                    ...
                    Log.d("MainActivity", "⚠️ Falta Ingreso de Mercadería para $fecha")
                }
                if (!tiposExistentes.contains("Stock Final")) {
                    faltantes.add("Stock Final")
                    Log.d("MainActivity", "⚠️ Falta Stock Final para $fecha")
                }
                
                if (faltantes.isNotEmpty()) {
                    val fechaFormateada = formatearFechaParaMostrar(calendarFecha)
                    val problemaTexto = "📅 $diaSemana $fechaFormateada\n   ⚠️ Faltan: ${faltantes.joinToString(", ")}"
                    problemas.add(problemaTexto)
                    Log.d("MainActivity", "📅 Agregado problema para $fecha: ${faltantes.joinToString(", ")}")
                    Log.d("MainActivity", "📝 Lista de problemas ahora tiene ${problemas.size} elementos")
                } else {
                    Log.d("MainActivity", "✅ Fecha $fecha tiene todos los datos necesarios")
                }
            }
    ...
    /**
     * Verifica vencimientos de documentos y personal del local (igual que la web)
     * Genera alertas para documentos vencidos o por vencer (≤30 días)
     * y para libreta sanitaria / curso de manipulación del personal
     */
    private suspend fun verificarVencimientosDocumentosYPersonal(localId: String, problemas: MutableList<String>) {
        Log.d("MainActivity", "🔍 Verificando vencimientos de documentos y personal para local: $localId")
        
        try {
            val firestore = FirebaseRegionManager.getFirestore()
            val localRef = firestore.collection("locales").document(localId)
            
            // 1. Obtener documentos del local
            val docsSnapshot = localRef.collection("documentos").get().await()
            val documentos = docsSnapshot.mapNotNull { doc ->
                try {
                    doc.toObject(DocumentoLocal::class.java)
                } catch (e: Exception) {
                    Log.d("MainActivity", "Error parseando documento ${doc.id}: ${e.message}")
                    null
                }
            }
            
            Log.d("MainActivity", "📄 Documentos encontrados: ${documentos.size}")
            
            for (doc in documentos) {
                val dias = doc.diasParaVencer
                if (dias <= 0) {
                    val msg = if (dias == 0) "VENCE HOY" else "VENCIDO hace ${-dias} días"
                    problemas.add("📄 ${doc.nombreDocumento} - $msg")
                    Log.d("MainActivity", "  📄 ALERTA: ${doc.nombreDocumento} - $msg")
                } else if (dias <= 30) {
                    val msg = if (dias == 1) "vence mañana" else "vence en $dias días"
                    problemas.add("📄 ${doc.nombreDocumento} - por vencer ($msg)")
                    Log.d("MainActivity", "  📄 ALERTA: ${doc.nombreDocumento} - por vencer ($msg)")
                }
            }
            
            // 2. Obtener personal del local
            val personalSnapshot = localRef.collection("personal").get().await()
            val personal = personalSnapshot.mapNotNull { doc ->
                try {
                    doc.toObject(PersonalLocal::class.java)
                } catch (e: Exception) {
                    Log.d("MainActivity", "Error parseando personal ${doc.id}: ${e.message}")
                    null
                }
            }
            
            Log.d("MainActivity", "👤 Personal encontrado: ${personal.size}")
            
            for (p in personal) {
                val nombre = p.nombreCompleto.ifBlank { "Sin nombre" }
                
                // Libreta sanitaria
                p.libretaSanitaria?.let { libreta ->
                    val dias = libreta.diasParaVencer
                    if (dias <= 0) {
                        val msg = if (dias == 0) "VENCE HOY" else "VENCIDA hace ${-dias} días"
                        problemas.add("🩺 Libreta sanitaria de $nombre - $msg")
                        Log.d("MainActivity", "  🩺 ALERTA: Libreta sanitaria de $nombre - $msg")
                    } else if (dias <= 30) {
                        val msg = if (dias == 1) "vence mañana" else "vence en $dias días"
                        problemas.add("🩺 Libreta sanitaria de $nombre - por vencer ($msg)")
                        Log.d("MainActivity", "  🩺 ALERTA: Libreta sanitaria de $nombre - por vencer ($msg)")
                    } else {
                        // Vigente, sin alerta
                    }
                }
                ...
                p.cursoManipulacion?.let { curso ->
                    val dias = curso.diasParaVencer
                    if (dias <= 0) {
                        val msg = if (dias == 0) "VENCE HOY" else "VENCIDO hace ${-dias} días"
                        problemas.add("📚 Curso de $nombre - $msg")
                        Log.d("MainActivity", "  📚 ALERTA: Curso de $nombre - $msg")
                    } else if (dias <= 30) {
                        val msg = if (dias == 1) "vence mañana" else "vence en $dias días"
                        problemas.add("📚 Curso de $nombre - por vencer ($msg)")
                        Log.d("MainActivity", "  📚 ALERTA: Curso de $nombre - por vencer ($msg)")
                    } else {
                        // Vigente, sin alerta
                    }
                }
            ...
            Log.e("MainActivity", "❌ Error verificando vencimientos de documentos y personal: ${e.message}", e)
        }
    }
    ...
    /**
     * Obtiene el nombre del día de la semana en formato corto desde un Calendar
     * Usa la misma lógica que GestionLocalesActivity para mantener consistencia
     */
    ...
    /**
     * Formatea una fecha para mostrar en formato "DD-MM-YY" desde un Calendar
     * Usa la misma lógica que GestionLocalesActivity para mantener consistencia
     */
    ...
    private fun mostrarDetallesProblemas(nombreLocal: String, problemas: List<String>) {
        ...
        // Agregar problemas parseados (con fecha) como cards clickeables
        ...
        // Agregar problemas NO parseados (vencimientos, etc.) como cards simples
        val problemasNoParseados = problemas.filter { p ->
            problemasParseados.none { it.texto == p }
        }
        problemasNoParseados.forEach { problemaTexto ->
            val cardProblema = LinearLayout(this).apply {
                orientation = LinearLayout.VERTICAL
                layoutParams = LinearLayout.LayoutParams(
                    LinearLayout.LayoutParams.MATCH_PARENT,
        ...
        dialog.window?.apply {
            ...
            if (darkMode) {
                setBackgroundDrawableResource(R.color.web_text_primary)
                decorView.setBackgroundColor(0xFF1E1E1E.toInt())
            } else {
                setBackgroundDrawableResource(R.color.web_text_light)
                decorView.setBackgroundColor(resources.getColor(R.color.surface_color, null))
            }
            setLayout(
