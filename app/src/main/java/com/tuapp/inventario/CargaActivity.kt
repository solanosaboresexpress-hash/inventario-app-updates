...
class CargaActivity : AppCompatActivity() {
    ...
    private fun cargarProductosDesdeLocal() {
        lifecycleScope.launch {
            try {
                ...
                // Cargar productos locales primero (más rápido)
                localRepository.getAllProductos(localId).collect { productosLocales: List<Producto> ->
    ...
    private fun cargarProductosDesdeFirebase() {
        lifecycleScope.launch {
            try {
                ...
                firebaseRepo.getAllProductos().collect { productos: List<Producto> ->
                    if (isFinishing || isDestroyed) return@collect
    ...
    private fun cargarDatosExistentes() {
        ...
        lifecycleScope.launch {
            try {
                ...
                // Leer directamente de Firebase para obtener el valor más actualizado
                ...
                if (usuarioActual != null) {
                    ...
                    firebaseRepo.getRegistrosByFecha(fecha).collect { registrosFirebase: List<RegistroInventario> ->
                        if (isFinishing || isDestroyed) return@collect
                        ...
                        if (registroExistente != null) {
                            ...
                            // Hay datos existentes, cargarlos en la interfaz
                    ...
                    // Fallback a datos locales si no hay usuario
                    localRepository.getRegistrosByFecha(localId, fecha).collect { registros: List<RegistroInventario> ->
                        if (isFinishing || isDestroyed) return@collect
            ...
            if (categoriaLayout.childCount > 1) {
                ...
                for (j in 0 until productosLayout.childCount) {
                    ...
                    if (edtCantidad != null) {
                        ...
                        val productoExistente = productosExistentes.find { it.nombre == producto.nombre }
    ...
    private fun guardarDatos() {
        ...
        lifecycleScope.launch {
            try {
                ...
                // Guardar localmente primero (más confiable)
                ...
                // Esperar a que Room complete la transacción antes de continuar
                // Usar withContext para asegurar que la transacción se complete
                ...
                runOnUiThread {
                    ...
                    try {
                        ...
                        Log.d("CargaActivity", "📡 Broadcast enviado para actualizar botones")
                    } catch (e: Exception) {
                        Log.w("CargaActivity", "Error marcando datos guardados: ${e.message}")
                    }
                    
                    // Mostrar mensaje de confirmación más visible
                    android.widget.Toast.makeText(
                        this@CargaActivity, 
                    ...
                    // Cerrar la actividad después de un delay para que el usuario vea el mensaje
                    // Reducido a 1500ms ya que ahora esperamos a que Room complete la transacción
                    android.os.Handler(android.os.Looper.getMainLooper()).postDelayed({
                ...
                // Intentar sincronización inteligente con Firebase en background (no bloquea la UI)
                lifecycleScope.launch {
                    try {
                        val firebaseRepo = FirebaseInventarioRepository(localId)
                        ...
                        firebaseRepo.sincronizacionInteligente(localRepository)
                        
                        Log.d("CargaActivity", "Datos sincronizados exitosamente con Firebase")
                    } catch (e: Exception) {
                        Log.w("CargaActivity", "No se pudo sincronizar con Firebase: ${e.message}")
                    }
                }
                
            } catch (e: Exception) {
                Log.e("CargaActivity", "Error saving data", e)
                // Mostrar mensaje de error al usuario y restaurar el botón
                runOnUiThread {
                    btnGuardar.isEnabled = true
                    btnGuardar.text = "👁️ Previsualizar y Guardar"
                    android.widget.Toast.makeText(this@CargaActivity, "❌ Error al guardar los datos: ${e.message}", android.widget.Toast.LENGTH_LONG).show()
                }
            }
        }
    }
    ...
    private fun showSkeletonLoading(message: String = "Cargando datos...") {
        ...
        runOnUiThread {
            ...
            if (isDark) {
                ...
                txtOfflineMsg.setTextColor(android.graphics.Color.WHITE)
    ...
    private suspend fun verificarSiDebeProcesarDescuentos(
        ...
        return try {
            ...
            if (fecha < fechaHoy) {
                Log.d("VENCIMIENTOS", "⏭️ Stock Final para $fecha: FECHA ANTERIOR (${fecha} < ${fechaHoy}) - NO procesará descuentos")
            ...
            if (stockFinalExistente == null) {
                ...
                if (datosSonDiferentes) {
                    ...
                    Log.d("VENCIMIENTOS", "🔍 Stock Final para $fecha: EXISTE con MISMOS DATOS (recarga)")
            ...
            try {
                val fechaHoy = DateHelper.getDateFormat("yyyy-MM-dd").format(Date())
                if (fecha < fechaHoy) {
                    Log.d("VENCIMIENTOS", "⏭️ Error pero fecha es anterior - NO procesará descuentos")
                    return ResultadoVerificacion(debeProcesar = false, esPrimeraVez = false)
                }
            } catch (e2: Exception) {
    ...
    private suspend fun procesarDescuentosVencimientos(fecha: String, stockFinalProductos: List<Producto>) {
        try {
            ...
            val stockFinalPorProducto = stockFinalProductos.associate { producto ->
                ...
                // Normalizar código para que coincida con los lotes (PEPPERO -> PEPPER, MUZZARE -> MUZZA)
            ...
            // 1. Calcular venta por producto: Stock Final Ayer + Ingreso Hoy - Stock Final Hoy
            val ventasPorProducto = calcularVentasPorProducto(fecha, stockFinalProductos)
            
            Log.d("VENCIMIENTOS", "📊 Ventas calculadas: $ventasPorProducto")
            
            // 2. Procesar descuentos FIFO basados SOLO en la venta (lotes más viejos primero)
            Log.d("VENCIMIENTOS", "📊 Ventas a procesar: ${ventasPorProducto.size} productos")
            ventasPorProducto.forEach { (codigo, venta) ->
                Log.d("VENCIMIENTOS", "   - $codigo: $venta uds")
            }
            vencimientoManager.procesarDescuentosFIFO(localId, fecha, ventasPorProducto)
            
            // 3. SINCRONIZAR lotes con stock final DESPUÉS del descuento FIFO
            // Esto ajusta automáticamente cualquier diferencia que quede
            Log.d("VENCIMIENTOS", "🔄 Sincronizando lotes con stock final después del descuento FIFO...")
            vencimientoManager.sincronizarLotesConStockFinal(localId, fecha, stockFinalPorProducto)
            
            // 4. Validar consistencia final (después de sincronización)
            Log.d("VENCIMIENTOS", "🔍 Validando consistencia final...")
            stockFinalPorProducto.forEach { (productoCodigo, stockFinal) ->
                ...
                if (sumaLotes != stockFinal) {
                    ...
                    Log.e("VENCIMIENTOS", "❌ ERROR: Inconsistencia persistente en $productoCodigo:")
                    Log.e("VENCIMIENTOS", "   Stock Final: $stockFinal")
                    Log.e("VENCIMIENTOS", "   Suma Lotes: $sumaLotes")
                    Log.e("VENCIMIENTOS", "   Diferencia: $diferencia")
                    Log.e("VENCIMIENTOS", "   Lotes: ${lotes.map { "${it.cantidad}uds(vto${it.fechaVencimiento})" }}")
                } else {
                    Log.d("VENCIMIENTOS", "✅ $productoCodigo: Lotes coinciden con stock final ($stockFinal)")
                }
            }
    ...
    private suspend fun calcularVentasPorProducto(fecha: String, stockFinalProductos: List<Producto>): Map<String, Int> {
        return try {
            ...
            stockFinalProductos.forEach { productoFinal ->
                ...
                Log.d("VENCIMIENTOS", "      Venta calculada: $venta")
                ...
                if (venta > 0) {
                    ...
                    Log.d("VENCIMIENTOS", "      ✅ Se procesará descuento FIFO")
                } else if (venta < 0) {
                    Log.w("VENCIMIENTOS", "      ⚠️ Venta negativa (posible error en datos)")
                } else {
                    Log.d("VENCIMIENTOS", "      ℹ️ Sin venta (solo sincronización)")
                }
            }
    ...
    /**
     * Obtiene el día de la semana en formato "LUNES", "MARTES", etc.
     */
    private fun obtenerDiaSemana(fecha: String): String {
        return try {
            ...
            Log.e("PROMEDIO_GENERAL", "Error obteniendo día de la semana: ${e.message}")
            "LUNES"
        }
    }
